...
const GroupSixPackage = () => {
  ...
  const onBet = useCallback(() => {
    ConfirmModal.show({
      title: `${i18n.t('lottery.threeStars')}${'组六包号'}`,
      data,
      betList,
      lotteryType,
    });
  ...
  const disabledRule = useCallback(() => {
    return data.every((item, index) => {
      return Array.isArray(betList[index]) && betList[index].length > 2;
    });
  }, [betList, data]);
  return (
    <View style={GStyle.container}>
      <TextL style={styles.tipStyle}>
        至少选择3个号码，开奖号码后三位为组六号且包含在选号中即中奖160金币！
      </TextL>
      <BetBody
        onTool={onTool}
        betList={betList}
...
export default memo(GroupSixPackage);
